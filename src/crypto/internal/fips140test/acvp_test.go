...
func cmdCShakeMct(hFn func(N, S []byte) *sha3.SHAKE) command {
	return command{
		...
		handler: func(args [][]byte) ([][]byte, error) {
			message := args[0]
			minOutLenBits := binary.LittleEndian.Uint32(args[1])
			maxOutLenBits := binary.LittleEndian.Uint32(args[2])
			outputLenBits := binary.LittleEndian.Uint32(args[3])
			incrementBits := binary.LittleEndian.Uint32(args[4])
			customization := args[5]

			if outputLenBits < 16 {
				return nil, fmt.Errorf("invalid output length: %d", outputLenBits)
			}

			rangeBits := (maxOutLenBits - minOutLenBits) + 1
			if rangeBits == 0 {
				return nil, fmt.Errorf("invalid maxOutLenBits and minOutLenBits: %d, %d", maxOutLenBits, minOutLenBits)
			}
			...
			for i := 0; i < 1000; i++ {
				...
				h.Write(innerMsg)
				digest := make([]byte, outputLenBits/8)
				h.Read(digest)
				message = digest

				// Rightmost_Output_bits = Right(Output[i], 16);
				rightmostOutput := digest[(outputLenBits/8)-2:]
				...
				outputLenBits = minOutLenBits + ((uint32(rightmostOutputBE)%rangeBits)/incrementBits)*incrementBits
			...
			encodedOutputLenBytes := make([]byte, 4)
			binary.LittleEndian.PutUint32(encodedOutputLenBytes, outputLenBits)
...
func TestACVP(t *testing.T) {
	...
	const (
		// TODO(@cpu): restore once upstream supports KDA OneStepNoCounter, cSHAKE.
		// bsslModule    = "boringssl.googlesource.com/boringssl.git"
		// bsslVersion   = "v0.0.0-20250123161947-ba24bde161f7"
		bsslModule    = "github.com/cpu/acvptool"
		bsslVersion   = "v0.0.0-20250126153608-f5aff5b28be3"
